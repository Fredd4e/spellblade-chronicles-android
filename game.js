...
function renderActions() {
    ...
    if (loc === 'village') {
        html += `<button onclick="talkElder()" class="rpg-btn bg-zinc-800 hover:bg-zinc-700 p-3 rounded-2xl flex gap-3 items-center text-left"><i class="fas fa-user-tie text-amber-400 w-5"></i><span>Talk to Elder</span></button>`;
        html += `<button onclick="openShop()" class="rpg-btn bg-zinc-800 hover:bg-zinc-700 p-3 rounded-2xl flex gap-3 items-center text-left"><i class="fas fa-store text-emerald-400 w-5"></i><span>Visit Merchant</span></button>`;
        ...
        html += `<button onclick="showMap()" class="rpg-btn bg-emerald-900/70 p-3 rounded-2xl flex gap-3 items-center text-left"><i class="fas fa-map w-5"></i><span>World Map</span></button>`;
    } else if (loc === 'woods') {
...
function castSpell() {
    ...
    if (spells.length > 1) {
        // Simple: cast first available or Firebolt
...
function defend() {
    ...
    setTimeout(() => {
        ...
        updateStats();
        if (p.hp <= 0) { /* death handled in enemyAttack but simple */ }
    }, 450);
}
...
function renderCombatButtons() {
    ...
    makeBtn('Flee', flee, 'fa-running', 'bg-zinc-700 hover:bg-zinc-600');
}
...
function searchLoot() {
    if (state.inCombat) return;
    const now = Date.now();
    if (now - (state.lastLootTime || 0) < 6500) {
...
function showMap() {
    const m = $('map-modal');
    if (!m) {
        // Fallback if modal missing in HTML
...
console.log("game.js fully loaded - Spellblade Chronicles modular ready");